...
FROM attempt INNER JOIN question ON question.subject_id = attempt.subject_id
WHERE attempt_id = (
...
-- Задание 10: Студент прошел тестирование (то есть все его ответы занесены в таблицу testing), далее необходимо вычислить результат(запрос) и занести его в таблицу attempt для соответствующей попытки.  Результат попытки вычислить как количество правильных ответов, деленное на 3 (количество вопросов в каждой попытке) и умноженное на 100. Результат округлить до целого.
UPDATE attempt
SET result = (SELECT ROUND(SUM(is_correct)*100/3,2)
    FROM testing
        INNER JOIN answer ON testing.answer_id = answer.answer_id
    WHERE attempt_id = 8)
WHERE attempt_id = 8;
